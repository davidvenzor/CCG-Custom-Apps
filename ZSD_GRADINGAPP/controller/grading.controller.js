sap.ui.define([
	...
	return Controller.extend("CCGSDGRADINGAPP.ZSD_GRADINGAPP.controller.grading", {
		...
		_onRouteMatched: function (oEvent) {
			...
			var filtergg = [
				new sap.ui.model.Filter("ImFlag", sap.ui.model.FilterOperator.EQ, 'Y')
			];
			// Dropdowns
				...
				success: function (oData, oResponse) {
					...
					if (oData.results.length !== 0) {
						if (oData.results[0].Perfect10 === "X") {
							...
							// this.getView().byId("idGrade").setSelectedKey("10");
							this.getView().byId("idPristineChk").setSelected(false);
							this.perfectchk = "S";
		...
		onNext: function (event) {
			...
			var lvnograd = this.getView().byId("idNoGrade").getSelectedKey();

			if ((this.getView().byId("idInputAuto").getSelected() === true) && (lvnograd === "")) {
		...
		onSave: function (oEvent) {
			...
			if ((this.getView().byId("idPerChk").getSelected() === true) || (this.getView().byId("idPristineChk").getSelected() === true)) {
				...
				if(this.getView().byId("idPerChk").getSelected() === true){
					...
					lperfect = "X";
					lv_Pristine = "";
				}else if(this.getView().byId("idPristineChk").getSelected() === true){
					lgrade = "10";
					lv_Pristine = "X";
					lperfect = "";
				}
			...
			oModeln.read("/Grade_MARADATASet", {
				...
				success: function (oData, oResponse) {
					...
					if (oData.results.length !== 0) {
						...
						this.VcScCrossover = " ";
						this.VcScCrossover = oData.results[0].VcScCrossover;
						this.getImageDataP(oData.results[0].CertificateNo);
						this.Certid = oData.results[0].CertificateNo;
					}
					if (oData.results.length !== 0) {
						if (oData.results[0].ZThickHolderFlag === "X") {
		...
		getImageDataP : function(Certid){
			
			// OData Call to get Image URL
			var tableModel = new sap.ui.model.json.JSONModel();
			var imgfilter = [
				new sap.ui.model.Filter("CertType", sap.ui.model.FilterOperator.EQ, "sportsCards"),
				new sap.ui.model.Filter("CertId", sap.ui.model.FilterOperator.EQ, Certid)
			];

			var that = this;
			this.oModel.read("/Image_displaySet", {
				filters: imgfilter,
				success: function (oData, oResponse) {
					sap.ui.core.BusyIndicator.hide();
					tableModel.setData(oData);
			...
			oModeln.read("/Grade_MARADATASet", {
				...
				success: function (oData, oResponse) {
					...
					if (oData.results.length !== 0) {
						...
						this.VcScCrossover = " ";
						this.VcScCrossover = oData.results[0].VcScCrossover;
						this.getImageNext(oData.results[0].CertificateNo);
						this.Certid = oData.results[0].CertificateNo;
					}
					if (oData.results.length !== 0) {
						if (oData.results[0].ZThickHolderFlag === "X") {
				...
				success: function (oData, oResponse) {
					...
					if (oData.results.length !== 0) {
						if (oData.results[0].Perfect10 === "X") {
							...
							this.getView().byId("idPristineChk").setSelected(true);
							this.getView().byId("idPerChk").setSelected(false);
							this.getView().byId("idPristineChk").setEnabled(true);
						}
						else{
							this.getView().byId("idPerChk").setSelected(false);
							this.getView().byId("idPristineChk").setSelected(false);
				...
				success: function (oData, oResponse) {
					...
					if (oData.results.length !== 0) {
						if (oData.results[0].Perfect10 === "X") {
							this.perfectchk = "S";
							this.getView().byId("idPerChk").setSelected(true);
							//this.getView().byId("idGrade").setvalue("P10");
							this.getView().byId("idPristineChk").setSelected(false);
							this.getView().byId("idPerChk").setEnabled(true);
						}  else if(oData.results[0].Pristine10 === "X"){
							this.getView().byId("idPristineChk").setSelected(true);
		...
		onPerSelect: function (oEvent) {
			...
			if (this.getView().byId("idPerChk").getSelected() == true) {
				...
				this.getView().byId("idGrade").setValue("10");
		...
		handleRefreshPress: function (oEvent) {
			...
			sap.ui.getCore().byId("idPlayerf").setValue(" ");
			sap.ui.getCore().byId("idMaterialf").setValue(" ");
		...
		handleGoDlogPress: function (oEvent) {
			...
			var lfvPlayer = sap.ui.getCore().byId("idPlayerf")._lastValue;
			var lfvMatnr = sap.ui.getCore().byId("idMaterialf")._lastValue;
			var lfvAttr1 = sap.ui.getCore().byId("idInputAttr1f")._lastValue;
			var lfvAttr2 = sap.ui.getCore().byId("idInputAttr2f")._lastValue;
		...
		OnGradeSelCng : function(oEvent){
			...
			if(kgrade == "10" && (addText == "PRISTINE" || addText == "Pristine")){
				this.getView().byId("idPristineChk").setSelected(true);
				this.getView().byId("idPerChk").setSelected(false);
			}else if(kgrade == "10" && (addText == "PERFECT" || addText == "Perfect")){
				this.getView().byId("idPristineChk").setSelected(false);
				this.getView().byId("idPerChk").setSelected(true);
			}
			else{
		...
		onNoGradeChange: function (oEvent) {
			...
			if (lNoGrade !== "") {
				...
				this.getView().byId("idAutoGrade").setEnabled(false);
				this.getView().byId("idPerChk").setEnabled(false);
				this.getView().byId("idPristineChk").setEnabled(false);
			}
			if (lNoGrade === "") {
				this.getView().byId("idGrade").setEnabled(true);
				this.getView().byId("idCenter").setEnabled(true);
		...
		_filter: function () {
			...
			this.byId("idGradeTablef").getBinding().filter(oFilter, "Application");
		},

		clearAllFilters: function (oEvent) {
			var oTable = this.byId("idGradeTablef");
